...
class MADDPG:
    def __init__(self, act_dim ,n_agents, dim_obs, dim_act, batch_size,
        ...
        if ifload:
            for i in range(n_agents):
                name1 = "parameter/actor_"+setting["actor_name"]+str(i)+".pth"
                name2 = "parameter/critic_"+setting["critic_name"]+str(i)+".pth"
                #print(name1)
    ...
    def update_policy(self):
        ...
        if self.steps_done % 30 == 0 and self.steps_done > 0:
            for i in range(self.n_agents):
                ...
                if self.steps_done % 600 == 0 and ifsave: 
                    th.save(self.critics[i].state_dict(),"parameter/critic_"+self.setting["actor_name"]+str(i)+".pth")
                    th.save(self.actors[i].state_dict(),"parameter/actor_"+self.setting["critic_name"]+str(i)+".pth")
    ...
    def select_action(self,state_batch,rule_prob):
        ...
        for i in range(self.n_agents):
            ...
            actProb = [x for x in act.detach()[0]]
            #print("rule_prob..",rule_prob[i].detach().numpy())
            #print("actProb..",actProb)
            new_action_prob = self.getLaw(rule_prob[i].detach().numpy(),actProb)
