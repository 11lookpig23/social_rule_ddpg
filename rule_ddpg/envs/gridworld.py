...
class Agent():
    ...
    def birth(self,busy):
        #print("birth self.pos *** ",self.pos,"birth self.id *** ",self.id)
        self.empty = False
        #self.id = id
        self.busy = busy
        self.step = 0
...
class Grid():
        ...
        for i in range(self.height):
            for j in range(2):
                if self.world[i,j].id!="none":
                    ### 还有waitList .... 
                    self.world[i,j].move = action_n[self.namelist.index(self.world[i,j].id)]
    ...
    def run_a_step(self,action_n):
       ...
       # print(" = true_act_n =",true_act_n)
       # print(" = reward_n =",reward_n)
       # print(" = after busy =",self.busy_n)
       # print(" obs_n ===  ",obs_n)
